...
def test_weather_penalty_reduces_both_soccer_rates():
    ...
    assert storm[0] < baseline[0]
    assert storm[1] < baseline[1]


def test_weather_is_ignored_for_basketball():
    baseline = expected_scores("basketball", 1550, 1500)
    wet = expected_scores("basketball", 1550, 1500, adjustments={"weather_penalty": 0.20})
    assert wet == pytest.approx(baseline)
...
def test_provider_values_are_bounded():
    ...
    assert extreme[0] >= baseline[0] * 0.385
    assert extreme[1] >= baseline[1] * 0.385
